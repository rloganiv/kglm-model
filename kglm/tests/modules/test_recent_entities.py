...
class RecentEntitiesTest(AllenNlpTestCase):
    ...
    def test_mask(self):
        ...
        # The mask for entity 3 should be 1 for the 2nd and 3rd timestep
        expected_3 = torch.tensor([0, 1, 1], dtype=torch.uint8)
        assert candidate_mask[0,:,3].equal(expected_3)
        # The mask for entity 4 should only be 1 for the last timestep
        expected_4 = torch.tensor([0, 0, 1], dtype=torch.uint8)
        assert candidate_mask[0,:,4].equal(expected_4)

        # Let's check that the remainders are correct
        # Entity 4 should have a remainder of 1, and everything else should be filtered out.
        assert self.recent_entities._remaining[0] == {4: 1}

        # If we run on the same data again, the remainder for entity 4 should activate the mask for
        # the first timestep.
        candidate_ids, candidate_mask = self.recent_entities(entity_ids_t0)
        expected_4 = torch.tensor([1, 0, 1], dtype=torch.uint8)
        assert candidate_mask[0,:,4].equal(expected_4)
